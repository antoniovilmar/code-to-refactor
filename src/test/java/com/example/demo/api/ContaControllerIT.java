...
public class ContaControllerIT extends BaseIT {
    ...
    void deveAbrirContaCorrente() throws Exception {
        ...
        final DadosAberturaContaDto dadosAberturaContaDto = new DadosAberturaContaDto(1, "99999");

        String numeroConta = mvc
                .perform(
                        post("/v1/conta-corrente")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(toJson(dadosAberturaContaDto)))
    ...
    void deveFazerUmDeposito() throws Exception {
        ...
        final Conta conta = new Conta("99999", TipoConta.CORRENTE, 1234, 1);
        this.contaRepository.save(conta);

        final MovimentacaoContaDto movimentacaoContaDto = new MovimentacaoContaDto(100, TipoMovimentacao.DEPOSITO);


        mvc.perform(
                        post("/v1/conta/{numeroConta}/movimentacao", conta.getNumero())
                                .contentType(MediaType.APPLICATION_JSON)
        ...
        assertAll("Comparando dados da movimentação de depósito",
                ...
                () -> assertEquals(conta.getNumero(), movimentacoes.get(0).getNumeroConta()),
                () -> assertEquals(conta.getTipoConta(), movimentacoes.get(0).getTipoConta()),
                () -> assertEquals(conta.getAgencia(), movimentacoes.get(0).getAgencia()),
    ...
    void deveFazerUmSaque() throws Exception {
        ...
        final MovimentacaoContaDto movimentacaoContaDto = new MovimentacaoContaDto(100, TipoMovimentacao.SAQUE);
    ...
    void deveGerarExtrato() throws Exception {
        ...
        final Conta conta = new Conta("99999", TipoConta.CORRENTE, 1234, 1);
        final Movimentacao movimentacaoSaque = new Movimentacao(1234, 1, -50, 50, TipoConta.CORRENTE, TipoMovimentacao.SAQUE);
        final Movimentacao movimentacaoDeposito = new Movimentacao(1234, 1, 50, 100, TipoConta.CORRENTE, TipoMovimentacao.DEPOSITO);
        this.contaRepository.save(conta);
        this.movimentacaoRepository.saveAll(List.of(movimentacaoSaque, movimentacaoDeposito));

        mvc.perform(
                        get("/v1/conta-extrato/{numeroConta}", conta.getNumero())
                                .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(jsonPath("$", hasSize(2)))
                .andExpect(jsonPath("$.[*]", hasItems(movimentacaoSaque.toString(), movimentacaoDeposito.toString())))
